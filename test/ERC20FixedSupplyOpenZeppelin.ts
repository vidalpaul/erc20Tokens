...
describe('ERC20FixedSupplyOpenZeppelin', async () => {
    ...
    it('Should transfer 200 Fix', async () => {
        ...
        await fixedSupply.transfer(addr2.address, 150);
        expect(await fixedSupply.balanceOf(addr1.address)).to.equal(200);
        expect(await fixedSupply.balanceOf(addr2.address)).to.equal(800);
    })
});
